document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!countryContentDiv || !schoolContentDiv) {
        console.error("Required DOM elements not found");
        return;
    }

    countryContentDiv.innerHTML = "<div class='loading-placeholder'>載入中...</div>";
    schoolContentDiv.innerHTML = "<div class='loading-placeholder'>載入中...</div>";

    fetch("data/School_data.json")
        .then(response => {
            console.log("School data response status:", response.status);
            if (!response.ok) {
        ...
        .catch(error => {
            console.error("載入學校數據失敗：", error);
            if (countryContentDiv) countryContentDiv.innerHTML = "<div class='loading-placeholder' style='color: red;'>載入失敗，請重新整理頁面</div>";
            if (schoolContentDiv) schoolContentDiv.innerHTML = "<div class='loading-placeholder' style='color: red;'>載入失敗，請重新整理頁面</div>";
        });
});
